...
# --- 1. 頁面配置 ---
st.set_page_config(page_title="馬尼通訊 企劃排程系統 v14.1.2", page_icon="🐎", layout="wide")
...
# --- 3. 初始化數據與範本 (已清除 cite 標記) ---
if 'templates_store' not in st.session_state:
    st.session_state.templates_store = {
        "🐎 馬年慶：百倍奉還": {
            "name": "2026 馬尼通訊「馬年慶：百倍奉還」活動執行企劃案 [cite: 1, 2]",
            "purpose": "迎接 2026 農曆馬年（丙午年），結合春節紅包與「百倍奉還」話題 [cite: 4]。透過 $100 元低門檻吸引新(舊)客戶，增加會員登錄與官網流量 [cite: 5]。",
            "core": "執行單位: 馬尼行動通訊門市 [cite: 9]；對象: 所有門市消費者 [cite: 8]；核心產品: 「百倍奉還」新年禮包 ($100/包) [cite: 10]。",
            "schedule": "宣傳期: 115/01/12-01/18 [cite: 12]\n銷售期: 115/01/19-02/08 [cite: 12]\n開獎日: 115/02/11 [cite: 12]\n兌獎期: 115/02/12-02/28 [cite: 12]",
            "prizes": "Sony PS5 (1名) [cite: 15] | 現金 $6,666 (1名) [cite: 15] | 總獎值突破 $130,000 [cite: 13]\n官網購物金 $1,500 | 115名 | 帶動二次消費 [cite: 17]",
            "sop": "確認客購數量(上限3包) [cite: 19]；告知序號保存 [cite: 20]；限量管理(每店66包) [cite: 21]；引導加入官方LINE [cite: 22]。",
            "marketing": "FB/IG/Threads 倒數限時動態 [cite: 25]；針對弱勢分店進行區域廣告投遞 [cite: 58]。",
            "risk": "稅務申報(>$1000) [cite: 28]；序號防偽蓋章 [cite: 31]；銷售分佈不均調度機制 [cite: 42]。",
            "effect": "預計帶動 2,000+ 人次進入門市 [cite: 34]；購物金帶動至少 60 筆官網訂單 [cite: 35]。"
        }
    }
    ...
    with col_tpl1:
        if st.button("📥 載入範本"):
            ...
            for k, v in data.items():
                # 載入時自動清理 cite 標籤，確保內文乾淨
                st.session_state[f"p_{k}"] = re.sub(r'\', '', str(v)).strip()
            st.rerun()
